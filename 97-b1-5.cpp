...
int hanno_move(HANDLE hout,int step,int x,int y,int z,int a[],int b[],int c[])
{
    ...
    if (step==1)
    {
		...
		gotoxy(hout,0,25);
        do{
            showch(hout,0,25,0,0,' ',40);
            showch(hout,0,26,0,0,' ',40);
            gotoxy(hout,0,25);
            setcolor(hout, 0, 7);
            cout<<"请输入操作(两个字母表示移动):"<<endl;
            cin>>input;
            if(cin.fail()||input[0]<'A'||input[0]>'z'||input[1]<'A'||input[1]>'z')
            {
                showch(hout,30,25,0,0,' ',30);
                gotoxy(hout,40,25);
                setcolor(hout, 0, 7);
                PlaySound("C:\\ERROR.wav", NULL, SND_FILENAME | SND_ASYNC );
                cout<<"输入非法,请重新输入"<<endl;
                setcolor(hout, 0, 7);
                gotoxy(hout,0,25);
                cin.clear();
            }
            else if((input[0]!=x+'A'-1&&input[0]!=x+'a'-1)||(input[1]!=z+'A'-1)&&(input[0]!=z+'a'-1))
            {
                showch(hout,30,25,0,0,' ',30);
                gotoxy(hout,40,25);
                setcolor(hout, 0, 7);
                cout<<"再动动脑子"<<endl;
                PlaySound("C:\\THINK.wav", NULL, SND_FILENAME | SND_ASYNC );
                setcolor(hout, 0, 7);
                gotoxy(hout,0,25);
            }
		}while((input[0]!=x+'A'-1&&input[0]!=x+'a'-1)||(input[1]!=z+'A'-1)&&(input[0]!=z+'a'-1));

        PlaySound("C:\\OK.wav", NULL, SND_FILENAME | SND_ASYNC );
		hanno_func(hout,a,c);
		hanno_traverse3(hout);
		showch(hout,40,5,0,0,' ',30);
		setcolor(hout, 0, 7);
		gotoxy(hout,40,5);
...
{
    do{
        PlaySound("C:\\dongbeiwanniba.wav", NULL, SND_FILENAME | SND_ASYNC );
    }while(1);
