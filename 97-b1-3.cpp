...
{
    width++;
    showch(hout,x-width,y,tower_color_set[width],0,' ',2*width+1);
}
...
void single_movement(HANDLE hout,int tower1[],int tower2[],int i,int j)
{
    ...
    int x,y;
	for (y = Y-i; y>ymax; y--) {
		draw_single_plate(hout,tower1[10],y,tower1[i-1]);
		Sleep(100);
		if (y>ymax) {
            draw_single_plate_black(hout,tower1[10],y,tower1[i-1]) ;
    ...
    if (tower1[10]<tower2[10])
    {
        ...
        draw_single_plate(hout,x,y,tower1[i-1]);
        Sleep(100);
        if (x<tower2[10]) {
            draw_single_plate_black(hout,x,y,tower1[i-1]);
            }
        }
    }
    else
    {
        ...
        Sleep(100);
        if (x>tower2[10]) {
            draw_single_plate_black(hout,x,y,tower1[i-1]);   //正常颜色：背景-0-黑色  前景-7-白色
            }
        }
    }
    for (y = ymax; y<=Y-j-1; y++) {
        draw_single_plate(hout,tower2[10],y,tower1[i-1]);
        Sleep(100);

        if (y<Y-j-1) {
            draw_single_plate_black(hout,tower2[10],y,tower1[i-1]);
            if (y>Y-column_height-1)
